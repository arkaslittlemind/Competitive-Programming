Question Link- https://atcoder.jp/contests/abc234/tasks/abc234_b

Problem Statement

There are
N points in a two-dimensional plane. The coordinates of the i-th point are
...
void solve() {
  ...
  for (int i = 1; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
      double curr = 0;
      curr = sqrt((x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j]));
      dist = max(dist, curr);
    }
  }
  cout << dist;
